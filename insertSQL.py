...
for i in range(1):
    ...
    name = xing[x]+ming[m1]+ming[m2]   # 生成姓名
    year = random.randint(1960,2000)
    month = random.randint(1,12)
    day = random.randint(1,29)
    birthday = f'{year}-{month}-{day}' # 生成生日
    age = 2019-year # 生成年龄
    emailList = ['qq.com','google.com','189.com','163.com']
    englishName = pypinyin.slug(name, separator='')
    email = englishName + '@' + emailList[random.randint(0,len(emailList)-1)] # 生成email
    phone = int(create_phone())
    USERLIST.append({'name':name,'age':age,'email':email,'gender':random.choice('男女'),'age':age,'phone':phone,'birthday':birthday})
# print(USERLIST)

# 打开数据库连接
db = pymysql.connect("127.0.0.1", "root", "root", "cn_189", charset='utf8')
# 使用cursor()方法获取操作游标
cursor = db.cursor()
try:
   # 执行sql语句
   for user in USERLIST:
       sql = f"""INSERT INTO table_name(name,email,gender,age,phone,birthday) values("{user['name']}","{user['email']}","{user['gender']}",{user['age']},{user['phone']},"{user['birthday']}")"""
       print(sql)
       cursor.execute(sql)
   # 提交到数据库执行
   db.commit()
